...
class CashAccountService(
    ...
    private val schema: DatabaseSchema.CashAccountTable
): CashAccountActions {

    override fun generateId(): AccountID = idGenerator.newId

    override fun addAccount(account: CashAccount): AccountResponse {
        return try {
            db.beginTransaction()
    ...
    override fun getAccount(accountID: AccountID): CashAccount? {
        return try {
            val cursor = db.query(
                schema.TABLE_NAME,
                schema.columns,
    ...
    override fun getAllAccounts(): List<CashAccount> {
        return try {
            val accounts = mutableListOf<CashAccount>()
            val cursor = db.query(
                schema.TABLE_NAME,
                schema.columns,
                null,
    ...
    override fun updateAccount(account: CashAccount): AccountResponse {
        return try {
            db.beginTransaction()
            if (!hasAccount(account.id)) {
                db.setTransactionSuccessful()
