...
class ExpenseCategoryService(
    ...
    override fun addCategory(category: ExpenseCategory): CategoryResponse {
        return try {
            db.beginTransaction()
            val contentValues = ContentValues().apply {
                put(schema.ID, category.id)
                put(schema.NAME, category.name.value)
                put(schema.IS_INCOME, false)
            }
            ...
            if (result != -1L) {
                ...
                CategoryResponse.CATEGORY_ALREADY_EXIST
            }
        } catch (e: SQLiteException) {
            e.printStackTrace()
    ...
    override fun getCategory(categoryID: CategoryID): ExpenseCategory? {
        return try {
            val cursor = db.query(
                schema.TABLE_NAME,
                schema.columns,
            ...
            cursor.use {
                if (cursor.moveToFirst()) {
                    ...
                    ExpenseCategory(categoryID, SimpleName(name))
                } else {
                    null
                }
            }
        ...
        return try {
            ...
            cursor.use {
                while (cursor.moveToNext()) {
                    ...
                    val name = cursor.getString(cursor.getColumnIndexOrThrow(schema.NAME))
                    categories.add(ExpenseCategory(id, SimpleName(name)))
                }
            }
            categories
        } catch (e: SQLiteException) {
    ...
    override fun updateCategory( category: ExpenseCategory): CategoryResponse {
        return try {
            db.beginTransaction()
            val contentValues = ContentValues().apply {
                put(schema.NAME, category.name.value)
            ...
            val result = db.update(
                ...
                contentValues,
                "${schema.ID} = ? AND ${schema.IS_INCOME} = ?",
                arrayOf(category.id.toString(), "0")
            )
            if (result > 0) {
                db.setTransactionSuccessful()
